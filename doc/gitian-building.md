...
*Setup instructions for a Gitian build of Sexcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sexcoin
...
- [Building Sexcoin Core](#building-sexcoin-core)
...
Clone the git repositories for sexcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/sexcoin/sexcoin
...
in the sexcoin repository under 'Fetch and create inputs' to install sources which require
...
To build Sexcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the sexcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sexcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/sexcoin/sexcoin
...
For example:
```bash
URL=https://github.com/thrasher-/sexcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sexcoin=${COMMIT} --url sexcoin=${URL} ../sexcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sexcoin=${COMMIT} --url sexcoin=${URL} ../sexcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sexcoin=${COMMIT} --url sexcoin=${URL} ../sexcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the sexcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sexcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/sexcoin
SIGPATH=/some/root/path/sexcoin-detached-sigs

./bin/gbuild --url sexcoin=${BTCPATH},signature=${SIGPATH} ../sexcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sexcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sexcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sexcoin-osx-build.assert
```
...
[sexcoin/gitian.sigs.via](https://github.com/sexcoin/gitian.sigs.via/) repository, or if that's not possible create a pull
